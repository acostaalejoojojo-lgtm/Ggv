...
import { ImportedModel, CustomAvatarSwitcher } from './ModelLoaders';
      ...
      <ErrorBoundary fallback={<mesh><boxGeometry args={[1,1,1]} /><meshStandardMaterial color="red" wireframe /></mesh>}>
        <Suspense fallback={<mesh><boxGeometry args={[1,1,1]} /><meshStandardMaterial color="gray" wireframe /></mesh>}>
          <CustomAvatarSwitcher 
            ...
            idleUrl={config.customAnimations?.idleUrl}
            walkUrl={config.customAnimations?.walkUrl}
            jumpUrl={config.customAnimations?.jumpUrl}
            runUrl={config.customAnimations?.runUrl}
            jumpAnimUrl={config.customAnimations?.jumpAnimUrl}
            idleAnimUrl={config.customAnimations?.idleAnimUrl}
            extraAnimations={config.customAnimations?.extraAnimations}
            emotes={config.customAnimations?.emotes}
            isMoving={isMoving}
            isJumping={isJumping}
        ...
        <group position={[0, -0.5, 0]}>
            <ErrorBoundary fallback={<mesh><boxGeometry args={[1,1,1]} /><meshStandardMaterial color="red" wireframe /></mesh>}>
                <React.Suspense fallback={null}>
                    {globalAvatar?.url ? (
                        <CustomAvatarSwitcher 
                          modelUrl={globalAvatar.url} 
                          idleUrl={(globalAvatar as any).animations?.idleUrl}
                          walkUrl={(globalAvatar as any).animations?.walkUrl}
                          jumpUrl={(globalAvatar as any).animations?.jumpUrl}
                          runUrl={(globalAvatar as any).animations?.runUrl}
                          jumpAnimUrl={(globalAvatar as any).animations?.jumpAnimUrl}
                          idleAnimUrl={(globalAvatar as any).animations?.idleAnimUrl}
                          extraAnimations={(globalAvatar as any).animations?.extraAnimations}
                          emotes={(globalAvatar as any).animations?.emotes}
